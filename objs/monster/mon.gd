...
func _fixed_process(delta):
	...
	# Reading inputs - TODO:MOVE INPUT TO INPUT MANAGER
	var input_enabled = globals.isPlayerInputEnabled()
	var walk_left = Input.is_action_pressed("p"+var2str(player)+"_left") if input_enabled else false
	var walk_right = Input.is_action_pressed("p"+var2str(player)+"_right") if input_enabled else false
	var jump = Input.is_action_pressed("p"+var2str(player)+"_jump") if input_enabled else false
	var smash = Input.is_action_pressed("p"+var2str(player)+"_smash") if input_enabled else false
	var activate_powerup = Input.is_action_pressed("p"+var2str(player)+"_powerup") if input_enabled else false
	...
	if is_colliding():
		...
		if on_air_time == 0 and force.x == 0 and get_travel().length() < SLIDE_STOP_MIN_TRAVEL \
			and abs(velocity.x) < SLIDE_STOP_VELOCITY and get_collider_velocity() == Vector2():
